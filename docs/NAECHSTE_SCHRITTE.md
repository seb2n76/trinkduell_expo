...
| **R2-CORS prüfen** | Profilbild- und Beweisfoto-Upload gehen vom Browser DIREKT an R2. Der Bucket braucht deshalb eine CORS-Regel: `PUT` erlauben, `AllowedOrigins` = `https://webapp.trinkduell.com` und `https://cloud.trinkduell.com`, `AllowedHeaders` = `content-type`. Ohne sie blockt der Browser den Upload. Cloudflare-Dashboard — R2 — Bucket — Settings — CORS Policy |
