...
// ─── Local Admin Data Helpers ──────────────────────────────────────────────────

export const getAdminStatsLocal = async () => {
  const users = await getUsers();
  const drinks = await getDrinks();
  const logs = await getDrinkLogs();
  const posts = await getPosts();
  ...
  for (const log of logs) {
    ...
    if (drink) {
      totalVolumeMl += drink.volume;
      totalAlcoholGrams += calculateAlcoholGrams(drink.volume, drink.abv);
    }
  }
  ...
  return {
    ...
    server: {
      uptimeSeconds: 3600,
      memoryUsageMb: 42,
      nodeVersion: "v20.x (Local Engine)",
      isPgMode: false,
    },
    activeRoomsCount: 0,
  };
...
export const getAdminUsersLocal = async (q?: string, filter?: string) => {
  const users = await getUsers();
  let result = users.map((u) => ({
    id: u.id,
    name: u.name,
    email: u.email || `${u.name.toLowerCase()}@trinkduell.de`,
    avatar: u.avatar || null,
    points: u.points || 0,
    level: u.level || 1,
    rank: u.rank || "Unranked",
    title: u.title || "Neuling",
    banned: false,
    alcoholGrams: u.alcoholGrams || 0,
...
export const banUserLocal = async (userId: string, banned: boolean) => {
  return { success: true, userId, banned };
};

export const resetUserStatsLocal = async (userId: string) => {
  const users = await getUsers();
  const user = users.find((u) => u.id === userId);
  if (user) {
    user.points = 0;
...
export const cleanUserProfileLocal = async (userId: string, resetName?: string) => {
  const users = await getUsers();
  const user = users.find((u) => u.id === userId);
  if (user) {
...
export const adminDeletePostLocal = async (postId: string) => {
  const posts = await getPosts();
  const filtered = posts.filter((p) => p.id !== postId);
  await saveRawData(KEYS.POSTS, filtered);
  return { success: true, id: postId };
};

export const adminUpdateDrinkLocal = async (drinkId: string, updates: Partial<Drink>) => {
  const drinks = await getDrinks();
  const drink = drinks.find((d) => d.id === drinkId);
  if (drink) {
    if (updates.name !== undefined) drink.name = updates.name;
    if (updates.volume !== undefined) drink.volume = updates.volume;
    if (updates.abv !== undefined) drink.abv = updates.abv;
    if (updates.calories !== undefined) drink.calories = updates.calories;
    if (updates.ean !== undefined) drink.ean = updates.ean;
    if (updates.hidden !== undefined) drink.hidden = updates.hidden;
    await saveRawData(KEYS.DRINKS, drinks);
    return drink;
  }
...
export const sendAdminBroadcastLocal = async (message: string) => {
  const post = await createPost(
    `📢 [SYSTEM-MITTEILUNG]: ${message}`,
    "friends",
    "friends",
    undefined
  );
